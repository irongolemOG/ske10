    ...
    <>
      ...
      >
        <div className="container mx-auto px-6 max-w-7xl flex items-center justify-between">
          ...
          {/* Desktop Nav */}
          <div className={`hidden xl:flex items-center space-x-0 lg:space-x-1 rounded-full px-1 lg:px-2 py-1 ${scrolled ? 'bg-slate-50 border border-gray-100' : 'bg-white/80 backdrop-blur-md border border-gray-200 shadow-sm'}`}>
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="px-2 lg:px-3 py-2 text-xs xl:text-sm font-semibold text-gray-600 hover:text-skeBlue transition-colors relative group whitespace-nowrap"
              >
                {link.name}
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="xl:hidden text-gray-900 p-2"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 bg-white/95 backdrop-blur-3xl pt-36 px-6 xl:hidden"
          >
